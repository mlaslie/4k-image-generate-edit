...
@pytest.mark.anyio
async def test_process_and_apply_4k_dpi():
    img = Image.new("RGB", (1024, 1024), color="purple")
    raw_buf = io.BytesIO()
    img.save(raw_buf, format="PNG")
    raw_bytes = raw_buf.getvalue()

    processed_bytes, mime_type, (w, h), (dpi_x, dpi_y) = await process_and_apply_4k_dpi(raw_bytes, aspect_ratio="16:9")
    assert w == 3840
    assert h == 2160
    assert dpi_x == 300
    assert dpi_y == 300
...
async def test_generate_4k_image_tool(mock_exec):
    ...
    result = await generate_4k_image(
        prompt="A futuristic neon cyber city at night in 4k",
        aspect_ratio="16:9",
        tool_context=mock_tool_ctx
    )

    assert "Successfully generated 4K image" in result
    ...
    assert saved_args.kwargs["filename"].startswith("generated_4k_")
    assert saved_args.kwargs["custom_metadata"]["dpi"] == "300x300"
    assert saved_args.kwargs["custom_metadata"]["model"] == MODEL_NAME
    assert saved_args.kwargs["custom_metadata"]["user_id"] == "artist@agency.com"

@pytest.mark.anyio
@patch("image_agent.tools._execute_gemini_3_pro_image")
async def test_edit_4k_image_tool(mock_exec):
    orig_img = Image.new("RGB", (1024, 1024), color="red")
    orig_buf = io.BytesIO()
    ...
    edited_img = Image.new("RGB", (1024, 1024), color="yellow")
    ...
    assert "Successfully edited image at 4K resolution" in result
    assert "3840x2160" in result
    assert "300 DPI" in result
    assert mock_tool_ctx.save_artifact.called
    saved_args = mock_tool_ctx.save_artifact.call_args
    assert saved_args.kwargs["custom_metadata"]["model"] == MODEL_NAME
    assert saved_args.kwargs["custom_metadata"]["user_id"] == "designer@agency.com"
