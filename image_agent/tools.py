...
def _get_session_id(tool_context: Optional[ToolContext] = None) -> str:
    """Extracts session_id from tool_context if available."""
    if not tool_context:
        return ""
    if hasattr(tool_context, "session_id") and tool_context.session_id:
        return str(tool_context.session_id)
    if hasattr(tool_context, "session") and tool_context.session:
        sid = getattr(tool_context.session, "session_id", None) or getattr(tool_context.session, "name", None)
        if sid:
...
async def _resolve_source_image_uri(
    ...
    """
    Robustly locates the source image URI or bytes across all possible upload/generation locations:
    1. Direct gs:// URI if provided
    2. gs://{GCS_BUCKET_NAME}/artifacts/{image_artifact_name} (if generated by agent)
    3. gs://{GCS_BUCKET_NAME}/image_agent/{user_id}/{session_id}/{image_artifact_name}/... (if uploaded in Gemini Enterprise)
    4. tool_context.load_artifact(image_artifact_name)
    5. Search in GCS user session path for uploaded photos
    6. Session events inline_data or file_data
    """
    session_id = _get_session_id(tool_context)
    client = get_storage_client()
    bucket = client.bucket(GCS_BUCKET_NAME) if client else None

    # Case 1: Direct gs:// URI
        ...
        try:
            ...
            if part:
                ...
                if hasattr(part, "inline_data") and part.inline_data and part.inline_data.data:
                    ...
                    uri = await _upload_to_gcs_async(temp_name, part.inline_data.data, part.inline_data.mime_type or "image/png")
            ...
            if content_obj and getattr(content_obj, "parts", None):
                for p in content_obj.parts:
                    ...
                    if hasattr(p, "inline_data") and p.inline_data and p.inline_data.data:
                        temp_name = f"upload_{int(time.time())}.png"
...
async def generate_4k_image(
    ...
    Saves image directly to GCS (NO tool_context.save_artifact to avoid artifact_delta echoes in GE).
...
async def edit_4k_image(
    ...
    Seamlessly locates user uploaded photos or previously generated images in GCS.
    ...
    Args:
        ...
        image_artifact_name: The filename/artifact name or gs:// URI of the base image to modify.
                            If omitted or uploaded by user, it will be automatically located from the session.
